...
import { TaskCardStatic } from './TaskCardStatic';
...
interface KanbanColumnStaticProps {
  id: string;
  title: string;
...
export function KanbanColumnStatic({
  ...
  if (isCollapsed) {
    return (
      ...
      >
        ...
        <span className="text-xs font-medium [writing-mode:vertical-rl] rotate-180 whitespace-nowrap">{title}</span>
  ...
  return (
    <div className={cn('flex flex-col h-full rounded-xl p-2 -m-2 min-w-0 min-h-0 border-2 border-transparent', 'transition-all duration-300 ease-out')}>
      <div className={cn('flex items-center gap-2 mb-3 shrink-0 bg-background z-10 py-1 rounded-lg transition-all duration-200')}>
        <Icon className={cn('w-4 h-4 shrink-0 transition-transform duration-200', config.iconColor)} />
        <h2 className="font-semibold text-sm truncate">{title}</h2>
        <span className={cn('text-xs px-1.5 py-0.5 rounded-full shrink-0 transition-all duration-200', config.badgeClass)}>
          {tasks.length}
        </span>
        <Button
          variant="ghost"
        ...
        {visibleTasks.length === 0 && tasks.length === 0 ? (
          <Card className="border-dashed transition-all duration-300">
            <CardContent className="p-3 text-center text-muted-foreground">
              <p className="text-xs transition-all duration-200">Không có task</p>
            </CardContent>
          </Card>
        ) : (
          <>
