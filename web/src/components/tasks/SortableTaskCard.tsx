  ...
  if (isCompactView) {
    return (
      <div
        ...
        {...listeners}
        className={cn(
          'flex items-center gap-2 p-1.5 rounded border-l-2 bg-card hover:bg-secondary/50 cursor-grab transition-all',
          isDone && 'opacity-60',
          isDragging && 'opacity-50 scale-95',
          isDragOverlay && 'shadow-2xl ring-2 ring-primary/30 rotate-2',
      ...
      >
        ...
        >
          {isDone ? (
            ...
            <Circle className={cn('w-3.5 h-3.5', task.priority === 'high' ? 'text-destructive' : task.priority === 'medium' ? 'text-warning' : 'text-muted-foreground')} />
          )}
        </button>
        <span className={cn('text-xs truncate flex-1', isDone && 'line-through text-muted-foreground')}>{task.title}</span>
        {task.dueDate && (
          <span className={cn('text-[9px] shrink-0', parseISO(task.dueDate) < today && !isDone ? 'text-destructive' : 'text-muted-foreground')}>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragging && 'scale-95 rotate-1 z-50',
        isDragOverlay && 'shadow-2xl ring-2 ring-primary/30 rotate-2 animate-scale-in'
      )}
    >
      <Card 
        className={cn(
